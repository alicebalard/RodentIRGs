  ...
  makePlotPerSpecies <- function(df){
    ggplot(df)+
      ...
      # genes
      geom_rect(aes(xmin=geneStartPlot, xmax=geneEndPlot, ymin=rankChr-0.2,ymax=rankChr+0.2, fill=IrgGroup),
                col="black", alpha = 0.7) +
  ...
  pdf(file = "../figures/SyntenyPlot_cricetidae.pdf", width = Wc, height = Hc)
