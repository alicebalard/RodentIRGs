  ...
  for (s in speciesList){
    ...
    for (c in chrList){
      ...
      if(paste0(s, c) %in% data$combi){
        ...
        # subdata$color[subdata$IrgName %in% "Irgb6-6*"] <- "pink"
        # subdata$color[subdata$IrgName %in% "Irgb10"] <- "green"
        # subdata$color[subdata$IrgName %in% "Irga2-6"] <- "yellow"
        
        group.colors <- c(IrgbTandems = "red", "Irgb6-6*" = "pink", Irgb10 ="green", "Irga2-6" = "yellow", 
        ...
        p <- ggplot(subdata)+
          ...
          geom_text(data = plot.labels, aes(x = xl, y = yl-0.07, label=labels), size = 3)+
          ## hline
          ...
          geom_segment(aes(x=max(end)+(max(end) - min(start))/div, xend = max(end)+ (max(end) - min(start))/div,
                           y = -0.75,  yend = -0.65)) +
          # gene
          geom_rect(aes(xmin=start, xmax=end, ymin=-0.5,ymax=+0.5, fill=IrgName), col="black", alpha = 0.7) +
...
pdf(file = "SyntenyPlot_muridae.pdf", width = 20, height =15)
makeplot(data[data$Species %in% c("M.musculus", "M.pahari","R.norvegicus","R.rattus"),])
dev.off()

pdf(file = "SyntenyPlot_cricetidae.pdf", width = 20, height =15)
makeplot(data[data$Species %in% c("M.ochrogaster","A.amphibius", "M.glareolus", "P.leucopus", "M.auratus"),])
dev.off()
